...
void test_xcrypt_ctr(const char* xcrypt)
{
	...
	// Loading file data to GPU global memory
	...
	// copying back encrypted data to the CPU
        cudaMemcpy(file_dat, d_file, file_size, cudaMemcpyDeviceToHost);
	// writing it to encrypted.txt
    ...
    // Decryption part - similar to encryption
    else {
        AES_init_ctx_iv(&ctx, key, iv);
